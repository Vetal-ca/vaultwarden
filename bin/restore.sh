...
pod_name="restore"
image="ttionya/vaultwarden-backup:1.15.0"
...
OPTS=$(getopt --options "h" --longoptions archive:,release:,capacity:,chart:,storage-class:,namespace:,context:,help --name 'parse-options' -- "$@")
if [ $? != 0 ]; then
...
echo "Performing the restore, if you have a zip password on archive, it will be prompted during the resore ..."
